...
:local countryResourceList [$loadFromJson [$dataRipe "country-resource-list" ("resource=$resource")]];

:local asn ($countryResourceList->"data"->"resources"->"asn");

:foreach asnItem in=$asn do={

	:put "";
	:put "AS: $asnItem";

	:local asRoutingConsistency [$loadFromJson [$dataRipe "as-routing-consistency" ("resource=AS$asnItem")]];
	
	:local prefixes ($asRoutingConsistency->"data"->"prefixes")

	:foreach prefixesItem in=$prefixes do={
		:local prefix ($prefixesItem->"prefix");
		:put "PREFIX: $prefix";
	}
}
